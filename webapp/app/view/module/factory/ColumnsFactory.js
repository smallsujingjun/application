		...
		canReduceTitle : function(group, field) {
			if (field.text.indexOf(group.text) == 0) {
				field.text = field.text
						.slice(group.text.length).replace('(',
								'').replace(')', '').replace(
								'（', '').replace('）', '');
			...
			case 'Double':
				Ext.apply(
					field,
					{
						...
						xtype : 'numbercolumn',
						width : 110,
						renderer : fd.tf_isCurrency ? Ext.util.Format.monetaryRenderer
